#!/usr/bin/env python3
"""Compare detected stations with official reference lists."""
import json
...
for lid, official in OFFICIAL.items():
    ...
    for sid, s in data['stations'].items():
        if s['line'] == lid:
            detected.append((sid, s['name_zh'], s['x'], s['y']))
    # Sort by station ID (arc-length order)
    detected.sort(key=lambda x: int(x[0].split('-')[1].lstrip('B').lstrip('C')))

    det_names = [d[1] for d in detected]
    print(f"\n=== Line {lid}: detected {len(detected)} (official {len(official)}) ===")
    # Show matching
    maxlen = max(len(det_names), len(official))
    for i in range(maxlen):
        off = official[i] if i < len(official) else ""
        det = det_names[i] if i < len(det_names) else ""
        sid = detected[i][0] if i < len(detected) else ""
        status = "✓" if det == off else ("?" if det and off else "×")
        if det != off:
            print(f"  {i+1:3d}. {sid:10s} official={off:15s}  detected={det:15s} {status}")
